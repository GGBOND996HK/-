...
- 手机本地可保存可删除样本元数据，拍照页支持确认牌面、手动修正、标记重拍和导出训练 manifest。
- `src/vision/camerashDataset.js` 提供 Camerash 数据集 42 类标签到项目牌码的映射，便于把公开数据集转成统一训练 manifest。
- 当前识别范围优先覆盖自己的手牌、摸牌和明牌；整桌牌河识别放到下一阶段。

### 本地样本闭环

拍照页每次识别都会把照片路径、识别结果和用户反馈写入 App 文档目录：

- `确认牌面`：把本次识别作为可训练标签。
- `保存修正`：用用户修正后的手牌、摸牌、明牌和死牌重新生成推荐，并把修正结果作为最终标签。
- `标记重拍`：保留低质量样本记录，后续训练时可作为困难/失败样本分析。
- `导出样本`：生成 `mahjong-training-*.json`，包含确认、修正和重拍样本的统一 manifest。
...
- 敲麻和清混碰不是同一个口径；敲麻允许普通平胡，但胡牌前必须先敲 / 报听。
- 13 张 `0 向` 只显示为听牌，不标记为已胡；14 张真正成型才进入 `winning`。
- 中发白按当前桌规作为花处理，不进入普通手牌结构。
- 风刻 / 明风刻按 1 花估计，明风杠按 2 花估计。
